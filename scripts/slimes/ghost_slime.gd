...
func _ready() -> void:
	...
	# Run base slime setup (which will apply unified speed scaling with 2x cap)
	super._ready()

	# Ghost HP is now handled by GameManager's get_scaled_enemy_hp_by_type()
	# with 0.7x multiplier - no need to override here
...
func apply_level_behaviors(_level: int) -> void:
	"""Ghost ignores level behaviors - uses unified scaling from GameManager."""
	return
